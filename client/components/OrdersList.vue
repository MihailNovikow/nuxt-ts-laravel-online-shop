      ...
      <div class="col-md-2">
        ...
        >
          ...
          <template v-if="order.status === 'pending payment'">
            <option :value="{ idx: idx, order: order.id, action: 'undo' }"
              >undo order</option
            >
            <option :value="{ idx: idx, order: order.id, action: 're_payment' }"
              >repeat payment</option
            >
          </template>
          <option
            v-else
...
import { defineComponent, created, watch } from "vue";
